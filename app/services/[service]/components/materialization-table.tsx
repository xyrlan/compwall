...
const ProcessingTimeline = ({ range }: any) => {
  ...
  return (
    <div className="relative w-full h-1 ">
      <div
        className="absolute right-0 top-0 h-3 bg-teal-500 w-2/3"
      ></div>
    </div>
  );
};
...
const MaterializationTable = ({ rows, columns}: any) => {
  return (
    <Table aria-label="Example table with custom cells" radius='none' className='rounded-xl'>
      <TableHeader>
      ...
      <TableBody>
        {rows.map((row: any) => (
          <TableRow key={row.key}>
            ...
            <TableCell><ProcessingTimeline range={row.processingTimeline} /></TableCell>
